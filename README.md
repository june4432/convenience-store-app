...
### 👤 **회원 관리 & 포인트 시스템**
- **회원가입/로그인/로그아웃**: 사용자 계정 생성 및 관리
- **마이페이지**: 로그인한 사용자의 포인트 현황 및 적립 내역 확인
- **포인트 적립**: 결제 금액의 1% 자동 적립
- **QR 코드 포인트 적립**:
    - **고객용 QR 생성**: 마이페이지에서 본인 확인용 QR 코드 생성
    - **점원용 QR 스캔**: 매장 POS에서 고객 QR 스캔으로 포인트 적립 (비로그인 상태)
    - **자동 적립**: 로그인 상태에서 결제 시 자동 포인트 적립

### 🛒 **장바구니 & 결제**
- 세션 기반 장바구니
- QR코드 쿠폰 스캔 및 적용
- **고객 포인트 QR 스캔**: 비로그인 상태에서 고객 QR 스캔으로 포인트 적립
- 쿠폰 취소 기능
- 토스페이먼츠 연동 결제
- 결제 성공/실패 시 소리 재생
...
- `POST /login` - 로그인 처리
- `GET /logout` - 로그아웃 처리
- `GET /mypage` - 마이페이지 (사용자 정보 및 포인트 내역)
- `GET /generate_user_qr` - 사용자 포인트 적립용 QR코드 이미지 생성
- `POST /api/set_customer` - 스캔된 고객 정보 세션에 저장 (포인트 적립 대상 지정)
